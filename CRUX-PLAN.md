...
> **State as of 2026-08-20: Phase 3 is complete. `sift` is done and `salvage` runs.** It lives at `~/projects/crux`, outside the Obsidian vault, same as `waypoint` and `hone`, and it is its own git repository.
>
...
>
> **Next action: Phase 4**, salvage defect classes 6 to 10, ending on the hostile-PoC capstone of D19. The infrastructure for it already exists: `read_first` is recorded on every attempt from now, so the capstone has history to sit on rather than needing a retrofit.
>
