...
|---|---|---|
| `lineage-intro` | tutorial | solvable and seed-stable; excused the drill-quality checks (it teaches the mechanic by being trivial) |
...
**The tutorial, and learning a game by playing it (added 2026-08-24).** Real-use feedback was that lineage is confusing to jump into: two passes of up-front explainer text did not fix it, because a priced graph is an interactive *game* and text does not teach a game. So `lineage-intro` (`tutorial=True`, ordered first, titled "Start here: how lineage works") is the smallest possible domain, two unmissable moves, no decoys and no cost trap, whose brief walks you through pressing enter and watching what changes. `validate.py` holds it to the checks that keep it *usable* (solvable, seed-stable) and excuses the ones that make a scenario a good *drill* (a cul-de-sac, a cost gap, resistance to a greedy player), and enforces at most one tutorial per track. Two supporting changes to the walk screen: every move now prints **"Took X for n"** feedback, because a graph that redraws in place gives a first-timer no sign that enter did anything; and the generic first-run explainer is suppressed on the tutorial, whose brief already teaches. This is the same lesson `test-tty.py` exists for, one level up: the thing you cannot verify by reading is whether a person can tell what the program just did.
...
which the six drill collections now use sixteen (the tutorial aside). The three unused kinds
...
| Date | What happened |
|---|---|
| 2026-09-06 | **Public.** The repository is public on GitHub under MIT (`LICENSE` added), with a release carrying `dist/crux.pyz`. The README gained *Getting it* (Python 3.10 or newer: the grammar parses at 3.8, `slots=True` dataclasses need 3.10), *For testers* (three evenings, what a useful report contains, the known edges) and a licence line. `validate.py` clean, `test.py` green at 59,946 checks. Waypoint stays private. |
| 2026-08-24 (e) | **A lineage tutorial, from "I have no idea what's going on" (still v1.7.0).** Real-use feedback, twice, that lineage is confusing to jump into. Two passes of up-front explainer text had not fixed it, and the reason is that a priced graph is an interactive *game*: text teaches a fact, playing teaches a game. So `lineage-intro` ("Start here: how lineage works", ordered first) is the smallest possible domain, two unmissable moves and no traps, whose brief walks you through pressing enter and watching what changes. A new `LineageBody.tutorial` flag; `validate.py` keeps the usability checks (solvable, seed-stable) and drops the drill-quality ones (cul-de-sac, cost gap, greedy resistance) for it, and enforces one tutorial per track. Two supporting walk-screen changes that help every scenario, not just the tutorial: each move now prints **"Took X for n"**, because a graph that redraws in place otherwise gives no sign enter did anything; and the generic first-run block is suppressed on the tutorial whose brief already teaches. Also fixed the tutorial's `you` node drawing a random name while its brief said `EXAMPLE\you` (pinned), and the feedback line truncating (wrapped). lineage is 7 collections; validate clean, test.py 59,947, test-tty.py 65, build ok. The general lesson, noted for next time: when a user says "I don't know what's going on," add a played tutorial and per-action feedback before adding more words. |
