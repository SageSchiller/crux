...
> **State as of 2026-08-20: Phase 4 is complete. `sift` and `salvage` are both done.** It lives at `~/projects/crux`, outside the Obsidian vault, same as `waypoint` and `hone`, and it is its own git repository.
>
...
>
> **Next action: Phase 5, the conduit spike.** Namespace supervisor lifecycle and teardown under crash and Ctrl-C, then sshd in a namespace. **The spike comes first and is allowed to fail**; D14 is the answer if it does. Nothing in sift or salvage is blocking.
>
