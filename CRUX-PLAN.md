...
>
> **Chain mode is the capstone and it works end to end.** The Wexler Corp engagement runs the three tracks as one continuous flow: read the scan for the pinned CMS, repair its broken exploit to land a shell, pivot from that shell to an internal database. Each stage is the real engine (a real `MarkScreen`, `SalvageScreen`, `ConduitScreen` with an `on_done` seam), so a chain leg is scored and verified exactly as the standalone version, and a clean run ends on "rooted".
>
> **The counts (from `validate.py`, which is authoritative):** 26 sift scenarios across seven families, 10 salvage defect classes, 7 conduit topologies, 1 chain engagement of 3 stages. `validate.py` clean with zero warnings and it runs every salvage and conduit and chain solution against a real target; `test.py` green at 19,754 checks including a full engagement driven stage to stage; `test-tty.py` green at 37 against a real pty. `dist/crux.pyz` builds and runs standalone.
>
