...
> **State as of 2026-08-20: crux is v1.0.0. All seven phases are done. Every track is built, and chain mode threads them into one engagement.** It lives at `~/projects/crux`, outside the Obsidian vault, same as `waypoint` and `hone`, and it is its own git repository.
>
...
>
> **The counts (from `validate.py`, which is authoritative):** 26 sift scenarios across seven families, 10 salvage defect classes, 7 conduit topologies, 1 chain engagement of 3 stages. `validate.py` clean with zero warnings; `test.py` green at 19,754 checks including a full engagement driven stage to stage; `test-tty.py` green at 37 against a real pty. `dist/crux.pyz` builds and runs standalone.
>
