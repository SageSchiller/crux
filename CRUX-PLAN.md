...
> **State as of 2026-08-20: Phase 1 is built, and its gate is open.** It lives at `~/projects/crux`, outside the Obsidian vault, same as `waypoint` and `hone`, and it is its own git repository.
>
> **The harness and the whole sift engine are built.** Track picker, the marking screen with its two beats and horizontal panning, the seeded fixture builder, scoring, history, help, and honest stubs for the two unbuilt tracks. Three families are authored: nmap, web content discovery, and `sudo -l`. `python3 validate.py` is clean with zero warnings, `python3 test.py` is green, `python3 test-tty.py` is green against a real pty, and `./build.sh` produces a `dist/crux.pyz` that runs standalone. The live counts are whatever `validate.py` prints; it is authoritative and cannot drift, so do not keep a table of them here.
>
> **The Phase 1 gate has NOT been closed, and only the author can close it.** The plan says Phase 1 ends when *the mechanic survives real use by the author*, and that is a judgement about whether marking lines is tense or tedious. Building it is not the same as playing it. **Run all eleven scenarios before Phase 2 is authored**, because if the mechanic needs to change it should change against eleven scenarios and not against forty.
>
> **Next action: play it, then Phase 2.** `python3 -m crux`, then sift. If the mechanic holds, Phase 2 is breadth: the remaining artifact families, and tuning `DECOY_WEIGHT` against real scores rather than the guess it currently is.
>
